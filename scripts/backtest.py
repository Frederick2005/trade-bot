"""
OPTIMIZED backtester with up to 10 concurrent trades.
Pre-computes all indicators upfront for maximum speed.
...
MAX_OPEN_TRADES  = 10       # allow up to 10 concurrent positions
...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """Pre-compute all indicators once — O(n) not O(n²)."""
    close  = df["close"]
    high   = df["high"]
    low    = df["low"]
    volume = df["volume"]
    ...
    df["ema50_slope"]     = (
        (df["ema50"] - df["ema50"].shift(4)) / df["ema50"].shift(4) * 100
    )
    return df
...
def get_trend_indicators(df_trend: pd.DataFrame, signal_time) -> dict | None:
    ...
    if pd.isna(row.get("ema200", float("nan"))):
        return None
...
def evaluate_signal(
    ...
    if not all([price, ema50_s, ema200_s, ema50_t, ema200_t, atr_s]):
        return None
    if rsi_s is None or (isinstance(rsi_s, float) and np.isnan(rsi_s)):
        return None

    if not (ema50_t > ema200_t):             return None
    if not (ema50_s > ema200_s):             return None
    if not (-1.5 <= price_vs_ema50 <= 1.5):  return None
    if not (42.0 <= rsi_s <= 62.0):          return None
    if not bullish:                           return None
    if body_pct < 0.35:                      return None
    if ema50_slope <= 0:                     return None
...
async def backtest_combo(
    ...
    logger.info(f"Computing indicators ({len(df_signal)} candles)...")
    df_signal = compute_indicators(df_signal)
    df_trend  = compute_indicators(df_trend)

    df_signal = df_signal.dropna(
        subset=["ema200", "rsi", "atr"]
    ).reset_index(drop=True)
    df_trend  = df_trend.dropna(subset=["ema200"]).reset_index(drop=True)

    logger.info(f"Running backtest ({len(df_signal)} candles, max {MAX_OPEN_TRADES} concurrent trades)...")
    ...
    for i in range(len(df_signal)):
        ...
        current_high = row_s["high"]
        current_low  = row_s["low"]
        ...
        for trade in open_trades:
            ...
            if exit_px is not None:
                pnl      = (exit_px - trade["entry"]) * trade["lot"]
        ...
        # ── Open new trade if slot available ────────────────────────
    ...
    if open_trades:
        last_price = df_signal.iloc[-1]["close"]
        for trade in open_trades:
            pnl      = (last_price - trade["entry"]) * trade["lot"]
    ...
    sharpe  = (
        pnl_arr.mean() / pnl_arr.std() * (252 ** 0.5)
...
async def main() -> None:
    logger.info("=" * 60)
    logger.info(f"OPTIMIZED BACKTEST — max {MAX_OPEN_TRADES} concurrent trades")
    logger.info("=" * 60)
    ...
    for combo in TIMEFRAME_COMBOS:
        ...
        if combo_results:
            ...
            all_results.append({
                "combo": combo, "results": combo_results, "score": score
            })
    ...
    for symbol in TRADING.symbols:
        await backtest_combo(
            symbol, best_combo["signal"], best_combo["trend"],
            save_labels=True,
        )
